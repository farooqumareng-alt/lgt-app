...
// Product.dimensions / Product.careInstructions migration to production
...
export async function POST(request: NextRequest) {
  ...
  if (action === "check") {
    const rows = await prisma.$queryRawUnsafe<{ column_name: string }[]>(
      `SELECT column_name FROM information_schema.columns WHERE table_name = 'Product' AND column_name IN ('dimensions', 'careInstructions');`,
    );
  ...
  if (action === "migrate") {
    await prisma.$executeRawUnsafe(
      `ALTER TABLE "Product" ADD COLUMN IF NOT EXISTS "careInstructions" TEXT, ADD COLUMN IF NOT EXISTS "dimensions" TEXT;`,
    );
